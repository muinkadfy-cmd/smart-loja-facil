...
import { type DelphiIconName, type DelphiIconSize } from '../lib/icons';
...
function iconTone(name: DelphiIconName): string {
  const toneMap: Partial<Record<DelphiIconName, string>> = {
    app_logo_cadeado_carrinho: '#1f6ed4',
    painel_da_loja: '#246fe5',
    produtos: '#0f68d8',
    categorias: '#4b63d6',
    clientes: '#7c3aed',
    usuario_administrador: '#2563eb',
    pedidos: '#d97706',
    vendas_pdv: '#0b6fd3',
    caixa: '#07813b',
    abrir_caixa: '#07813b',
    ...
    relatorios: '#5b21b6',
    backup: '#0f63c9',
    configuracoes: '#4754d8',
    auditoria_logs: '#1d4ed8',
    bloqueio_seguro: '#07813b',
    ajuda: '#315ccf',
    atualizar: '#0284c7',
    atalhos: '#334fd8',
    offline_local: '#07813b',
    sqlite_ativo: '#0b78c8',
    arquivo_banco_sqlite: '#0b78c8',
    buscar: '#315ccf',
    calendario_data: '#0f68d8',
    dinheiro: '#07813b',
    cartao_credito: '#0b78c8',
    cartao_debito: '#0b78c8',
    pix: '#059669',
    editar: '#c07606',
    excluir: '#dc2626',
    novo_item_adicionar: '#2563eb',
    remover_menos: '#d14d27',
    estoque_baixo: '#ea580c',
    fornecedores: '#0f68d8',
    imprimir: '#315ccf',
    loja_ativa: '#5b21b6',
    sistema_local: '#334fd8',
    transferir_estoque: '#0369a1',
    whatsapp: '#07813b',
  };

  return toneMap[name] ?? '#1d4ed8';
}
export function AppIcon({ name, size, alt = '', className = '' }: AppIconProps): JSX.Element {
  ...
  const gradientId = `smartIconGradient${rawId}`;
  const glowId = `smartIconGlow${rawId}`;
  const tone = iconTone(name);
  const label = alt || undefined;

  return (
    <span
      className={`app-svg-icon app-modern-icon ${className}`.trim()}
      style={{ width: `${size}px`, height: `${size}px`, color: tone }}
      ...
      <svg viewBox="0 0 24 24" width="100%" height="100%" focusable="false">
        <defs>
          ...
          <radialGradient id={glowId} cx="35%" cy="24%" r="72%">
            ...
            <stop offset="0.38" stopColor="#ffffff" stopOpacity="0.28" />
            <stop offset="1" stopColor="currentColor" stopOpacity="0" />
          </radialGradient>
        </defs>
        <rect x="2.4" y="2.4" width="19.2" height="19.2" rx="6.2" fill={`url(#${gradientId})`} opacity="0.22" />
        <rect x="3.2" y="3.1" width="17.6" height="17.6" rx="5.4" fill="#ffffff" opacity="0.46" />
        <path d="M5.1 6.1c2.2-2 6.5-2.8 11.2-.9" fill="none" stroke="#ffffff" strokeWidth="1.45" strokeLinecap="round" opacity="0.92" />
        <circle cx="18.2" cy="5.9" r="2.1" fill={`url(#${glowId})`} opacity="0.95" />
        <g color={tone} filter="none">
          {IconSvg({ name })}
        </g>
      </svg>
    </span>
