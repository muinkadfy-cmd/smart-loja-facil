...
export function SettingsPage({ refreshToken, settings, onSettingsSaved, onChanged }: PageProps): JSX.Element {
  ...
  return (
    <div className="stack">
      <div className="page-title"><h1>Configurações</h1><p>{runtimeInfo.isWeb ? 'Dados da loja sincronizados no Supabase para web/mobile.' : 'Dados da loja, comprovante A6 10,5 x 14,8 cm, estoque baixo e modo PC lento.'}</p></div>
      <section className="panel form-panel">
        <form className="form-grid" onSubmit={submit}>
          <label>Nome da loja<input value={form.store_name} onChange={(e) => setForm({ ...form, store_name: e.target.value })} /></label>
          <label>Responsável<input value={form.owner_name} onChange={(e) => setForm({ ...form, owner_name: e.target.value })} /></label>
          ...
          <button className="primary-btn"><AppIcon name="configuracoes" size={16} className="app-icon-button-inline" />Salvar configurações</button>
        </form>
      </section>
