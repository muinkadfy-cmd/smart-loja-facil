...
export function BackupPage({ refreshToken, onChanged }: PageProps): JSX.Element {
  ...
  const canRestore = !runtimeInfo.isWeb || permissions.canManageStore;
  ...
  async function restoreSaved(row: BackupInfo) {
    if (!canRestore) {
      setMessage('');
      setError(permissions.readonlyMessage || 'Seu perfil não permite restaurar backup.');
      return;
    }
    if (runtimeInfo.isWeb) {
  ...
  async function restoreExternal() {
    if (!canRestore) {
      setMessage('');
      setError(permissions.readonlyMessage || 'Seu perfil não permite importar/restaurar backup.');
      return;
    }
    if (runtimeInfo.isWeb) {
  ...
  async function importWebBackup(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!canRestore) {
      setMessage('');
      setError(permissions.readonlyMessage || 'Seu perfil não permite importar backup.');
      return;
    }
  ...
  return (
    <div className="stack backup-light-v65 backup-premium-v93">
      ...
      <section className="panel reports-hero-panel backup-dashboard-v93">
      ...
      <section className="panel backup-callout backup-callout-premium backup-main-action-v93">
        <div>
      ...
      <section className="backup-tips-grid backup-tips-v93">
      ...
      {runtimeInfo.isWeb && !canRestore ? <div className="web-readonly-module-note"><strong>{permissions.roleLabel}</strong><span>{permissions.readonlyMessage || 'Você pode baixar backup para consulta, mas importar/restaurar fica bloqueado para este perfil.'}</span></div> : null}

      <section className="panel backup-external-panel backup-import-v93">
        <div className="panel-head panel-head-tight">
          ...
          <button className="secondary-btn" onClick={restoreExternal} disabled={loading || !canRestore}><AppIcon name="backup" size={16} className="app-icon-button-inline" />{runtimeInfo.isWeb ? 'Importar JSON' : 'Escolher arquivo para restaurar'}</button>
        </div>
      </section>

      <section className="panel backup-history-v93">
        <div className="panel-head panel-head-tight">
          <div>
        ...
        <DataTable<BackupInfo>
          ...
          columns={[
            ...
            { key: 'date', label: 'Criado em', render: (row) => dateTime(row.created_at) },
            {
              ...
              label: 'Ação',
              align: 'right',
              render: (row) => <button type="button" className="secondary-btn small" onClick={() => restoreSaved(row)} disabled={loading || !canRestore}>{runtimeInfo.isWeb ? 'Orientação' : 'Restaurar'}</button>,
            },
          ]}
        />
      </section>
