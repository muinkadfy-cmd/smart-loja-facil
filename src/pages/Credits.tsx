...
const JAQUE_LOGO_PREMIUM_DATA_URI = '/brand/jaque-logo-premium.png';
  ...
  <html>
    ...
    <body>
      <main class="sheet">
        ...
        <header class="top">
          <div class="brand">
            <img class="brand-logo" src="${JAQUE_LOGO_PREMIUM_DATA_URI}" alt="Jaque Confeccoes e Presentes" />
          </div>
...
export function CreditsPage({ refreshToken, onChanged }: PageProps): JSX.Element {
  ...
  function openReceiveModal(credit: CreditSummary, installment: CreditInstallment) {
    if (!canOperate) {
      setMessage('');
      setError(permissions.readonlyMessage || 'Você não tem permissão para receber parcelas.');
      return;
    }
    setError('');
    setMessage('');
  ...
  async function submitReceive(event: FormEvent<HTMLFormElement>) {
    ...
    if (!canOperate) {
      setMessage('');
      setError(permissions.readonlyMessage || 'Você não tem permissão para confirmar recebimentos.');
      return;
    }
    setSaving(true);
    setError('');
  ...
  const summary = useMemo(() => {
    ...
    const installments = rows.flatMap((credit) => credit.installments);
    const overdue = installments.filter((item) => isInstallmentOverdue(item));
    const paidTotal = installments.reduce((sum, item) => sum + item.paid_amount, 0);
    const overdueBalance = overdue.reduce((sum, item) => sum + Math.max(0, item.amount - item.paid_amount), 0);
    const partial = installments.filter((item) => item.status === 'parcial').length;
    return {
  ...
  return (
    <div className="stack classic-legacy-page credits-light-v65 credits-premium-v91">
      <div className="page-title classic-legacy-title">
        <div>
          <h1>Crediário</h1>
          <p>Controle parcelas, valores pagos, restante, vencimentos, PDF e cobrança pelo WhatsApp com clareza.</p>
        </div>
      </div>

      <div className="credits-helper-v91"><strong>Fluxo do crediário</strong><span>1. Filtre o cliente · 2. Confira valor, pago e restante · 3. Receba a parcela ou gere PDF/WhatsApp.</span></div>
      ...
      <section className="stats-grid classic-stats-grid classic-credit-summary-grid">
        ...
        <article className="stat-card">
          <div className="stat-copy"><small>Saldo em aberto</small><strong>{money(summary.balance)}</strong></div>
        </article>
        <article className="stat-card">
          <div className="stat-copy"><small>Recebido</small><strong>{money(summary.paidTotal)}</strong></div>
        </article>
        <article className="stat-card">
          <div className="stat-copy"><small>Vencidas</small><strong>{summary.overdue}</strong></div>
        </article>
        <article className="stat-card">
          <div className="stat-copy"><small>Valor vencido</small><strong>{money(summary.overdueBalance)}</strong></div>
        </article>
      </section>
      ...
      <section className="panel classic-panel classic-legacy-table-panel credits-summary-panel-v91">
        ...
        <DataTable<CreditSummary>
          ...
          empty="Nenhum crediário encontrado. Quando vender no crediário, as parcelas aparecem aqui."
          columns={[
      ...
      {filteredCredits.map((credit) => (
        <section className="panel classic-panel classic-legacy-table-panel credit-customer-panel-v91" key={credit.id}>
          <div className="panel-head">
            <h2>{credit.customer_name}</h2>
            <div className="table-actions">
              <span className={creditStatusClass(credit.status)}>Restante {money(credit.balance)}</span>
              <span className="pill pill-info">Total {money(credit.total)}</span>
                ...
                render: (row) => (
                  <div className="table-actions">
                    ...
                    {row.status === 'pago'
                      ...
                      : canOperate ? <button type="button" className="secondary-btn small" onClick={() => openReceiveModal(credit, row)}>Receber</button> : <span className="muted">Consulta</span>}
                  </div>
                ),
              },
            ]}
      ...
      <Modal open={Boolean(receiving)} title="Receber parcela" onClose={() => !saving && setReceiving(null)}>
        {receiving && (
          <form className="form-grid compact receive-credit-form-v91" onSubmit={submitReceive}>
            ...
            <label>Valor recebido<input type="number" min="0.01" step="0.01" value={amount} onChange={(e) => setAmount(Number(e.target.value))} readOnly={!canOperate} /></label>
            <label>Forma<select value={method} onChange={(e) => setMethod(e.target.value as Exclude<PaymentMethod, 'crediario'>)} disabled={!canOperate}><option value="dinheiro">Dinheiro</option><option value="pix">Pix</option><option value="cartao">Cartão</option></select></label>
            <label className="span-2 check-row">
              <input type="checkbox" checked={redistribute} onChange={(e) => setRedistribute(e.target.checked)} disabled={!canOperate} />
            ...
            <div className="table-actions span-2">
              <button className="primary-btn" disabled={saving || !canOperate}>{saving ? 'Lançando...' : canOperate ? 'Confirmar recebimento' : 'Somente leitura'}</button>
            </div>
          </form>
        )}
      </Modal>
      ...
      <Modal open={Boolean(preview)} title="Comprovante da parcela" onClose={() => setPreview(null)}>
        {preview && (
          <>
            <div className="receipt-preview credit-receipt-preview-v91" dangerouslySetInnerHTML={{ __html: buildInstallmentHtml(settings, preview.credit, preview.installment) }} />
