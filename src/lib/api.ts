...
export const api = {
  ...
  backups: () => (isTauriRuntime() ? call<BackupInfo[]>('list_backups') : webBackups()),
  createBackup: () => (isTauriRuntime() ? call<BackupInfo>('create_backup') : webCreateBackup()),
  createBackupTo: (destinationDir: string) => (isTauriRuntime() ? call<BackupInfo>('create_backup_to', { destinationDir }) : webCreateBackup()),
  ...
  restoreWebBackupContent: (fileContent: string, confirmation: string) => webRestoreBackupContent(fileContent, confirmation),
