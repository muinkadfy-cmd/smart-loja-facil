...
export const WEB_APP_VERSION = 'pwa-supabase-v134-tour-comercial-guiado';
export const WEB_CACHE_VERSION = 'smart-loja-pwa-supabase-v134-tour-comercial-guiado';
...
export const WEB_TRAINING_MODE_KEY = 'smart-loja:training-mode-safe-v134';
const LEGACY_WEB_TRAINING_MODE_KEYS = ['smart-loja:training-mode-safe-v133', 'smart-loja:training-mode-safe-v132', 'smart-loja:training-mode-safe-v131'];
...
export const WEB_DEMO_MODE_KEY = 'smart-loja:demo-mode-safe-v134';
const LEGACY_WEB_DEMO_MODE_KEYS = ['smart-loja:demo-mode-safe-v133', 'smart-loja:demo-mode-safe-v132'];
...
function readGuidedCommercialProgress(): { done: number; total: number; percent: number } {
  ...
  try {
    const keys = ['smart-loja:guided-commercial-test-v134', 'smart-loja:guided-commercial-test-v133', 'smart-loja:guided-commercial-test-v131', 'smart-loja:guided-commercial-test-v130', 'smart-loja:guided-commercial-test-v129', 'smart-loja:guided-commercial-test-v128', 'smart-loja:guided-commercial-test-v127', 'smart-loja:guided-commercial-test-v126'];
...
function readAssistedCommercialProgress(): { passed: number; failed: number; blocked: number; total: number; percent: number; criticalProblems: number } {
  ...
  const keys = ['smart-loja:assisted-commercial-run-v134', 'smart-loja:assisted-commercial-run-v133', 'smart-loja:assisted-commercial-run-v131', 'smart-loja:assisted-commercial-run-v130', 'smart-loja:assisted-commercial-run-v129', 'smart-loja:assisted-commercial-run-v128', 'smart-loja:assisted-commercial-run-v127'];
  ...
  try {
    for (const key of keys) {
      ...
      for (const [id, result] of Object.entries(results)) {
        const normalizedId = id === 'deploy-cache-v128-real' || id === 'deploy-cache-v129-real' || id === 'deploy-cache-v130-real' || id === 'deploy-cache-v131-real' || id === 'deploy-cache-v133-real' ? 'deploy-cache-v134-real' : id;
...
function readFinalCommercialAcceptanceStatus(): { signed: boolean; responsible: string; acceptedAt: string; key: string } {
  const keys = ['smart-loja:final-commercial-acceptance-v134', 'smart-loja:final-commercial-acceptance-v133', 'smart-loja:final-commercial-acceptance-v131', 'smart-loja:final-commercial-acceptance-v130', 'smart-loja:final-commercial-acceptance-v129'];
...
function readCommercialTourProgress(): { done: number; total: number; percent: number; active: boolean } {
  const total = 8;
  const keys = ['smart-loja:commercial-tour-v134', 'smart-loja:commercial-tour-v133'];
...
export async function webCommercialValidation(): Promise<WebCommercialValidationReport> {
  ...
  pushCommercialCheck(checks, {
    id: 'training-mode-safe-v134', area: 'Treinamento', title: 'Modo treinamento seguro',
    ...
    evidence: trainingMode.enabled ? `responsavel=${trainingMode.responsible || 'não informado'}; cenario=${trainingMode.scenario || 'não informado'}` : 'smart-loja:training-mode-safe-v134 desligado',
  });

  pushCommercialCheck(checks, {
    id: 'demo-mode-separated-v134', area: 'Treinamento', title: 'Ambiente demo separado',
    ...
    evidence: demoMode.enabled ? `lojaDemo=${demoMode.storeName || 'Loja Demonstração Fácil'}; responsavel=${demoMode.responsible || 'não informado'}` : 'smart-loja:demo-mode-safe-v134 desligado',
  });


  const tourProgress = readCommercialTourProgress();
  pushCommercialCheck(checks, {
    id: 'commercial-tour-v134', area: 'Treinamento', title: 'Tour de apresentação comercial',
    ...
    evidence: `progresso=${tourProgress.percent}%; chave=smart-loja:commercial-tour-v134`,
  });
  ...
  pushCommercialCheck(checks, {
    ...
    detail: cacheKeys.includes(WEB_CACHE_VERSION) ? 'Cache novo v134 encontrado neste aparelho.' : 'Cache novo ainda não apareceu; pode precisar abrir após deploy ou limpar cache antigo.',
  ...
  pushCommercialCheck(checks, {
    id: 'guided-commercial-v134', area: 'Teste real', title: 'Roteiro guiado multiaparelho',
    ...
    evidence: `progresso=${guidedProgress.percent}%; chave=smart-loja:guided-commercial-test-v134`,
  });

  const assistedProgress = readAssistedCommercialProgress();
  pushCommercialCheck(checks, {
    id: 'assisted-execution-v134', area: 'Teste real', title: 'Execução real assistida',
    ...
    evidence: `passou=${assistedProgress.passed}; falhou=${assistedProgress.failed}; bloqueado=${assistedProgress.blocked}; chave=smart-loja:assisted-commercial-run-v134`,
  });


  const finalAcceptance = readFinalCommercialAcceptanceStatus();
  pushCommercialCheck(checks, {
    id: 'final-commercial-acceptance-v134', area: 'Teste real', title: 'Aceite final de venda',
    ...
    evidence: finalAcceptance.signed ? `chave=${finalAcceptance.key}; acceptedAt=${finalAcceptance.acceptedAt}` : 'chave=smart-loja:final-commercial-acceptance-v134 sem aceite registrado',
  });
