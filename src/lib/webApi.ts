...
export const WEB_APP_VERSION = 'pwa-supabase-v96';
export const WEB_CACHE_VERSION = 'smart-loja-pwa-supabase-v96-commercial-validation';
...
async function createFirstStore(userId: string, email: string): Promise<WebStoreContext> {
  ...
  if (error) {
    const hint = error.message.toLowerCase().includes('row-level security')
      ? ' Verifique se as migrations Supabase/RLS foram aplicadas e se a policy stores_insert_owner permite owner_id = auth.uid().'
      : '';
    throw new Error(`Não foi possível criar a loja web inicial: ${error.message}.${hint}`);
  }
