...
const currentVersion = 'pwa-supabase-v134-tour-comercial-guiado';
const currentCache = 'smart-loja-pwa-supabase-v134-tour-comercial-guiado';
...
if (!mainSource.includes('smart-mobile-rebuild-v134')) fail('main.tsx precisa aplicar a classe smart-mobile-rebuild-v134.');
...
if (!webApiSource.includes(currentCache)) fail('WEB_CACHE_VERSION precisa estar no cache v134 tour comercial guiado.');
if (!serviceWorkerSource.includes(currentCache)) fail('Service worker precisa usar cache v134 tour comercial guiado.');
...
for (const token of ['mapp-root', 'mapp-bottom-nav', 'mapp-sidebar', 'mapp-page', 'mapp-stat-card', 'mapp-alert-card', 'mapp-guided-test-panel', 'mapp-assisted-execution-panel', 'mapp-triage-panel', 'mapp-final-release-panel', 'mapp-demo-panel', 'mapp-tour-panel']) {
...
console.log('OK: release_check v134 PWA passou. Tour comercial guiado, modo treinamento, onboarding, aceite final, teste guiado, permissões por papel e Supabase preservado.');
