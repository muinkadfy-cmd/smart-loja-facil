...
const currentVersion = 'pwa-supabase-v129-correcao-pos-teste';
const currentCache = 'smart-loja-pwa-supabase-v129-correcao-pos-teste';
...
if (!mainSource.includes('smart-mobile-rebuild-v129')) fail('main.tsx precisa aplicar a classe smart-mobile-rebuild-v129.');
...
if (!webApiSource.includes(currentCache)) fail('WEB_CACHE_VERSION precisa estar no cache v129 correção pós-teste.');
if (!serviceWorkerSource.includes(currentCache)) fail('Service worker precisa usar cache v129 correção pós-teste.');
...
for (const token of ['mapp-root', 'mapp-bottom-nav', 'mapp-sidebar', 'mapp-page', 'mapp-stat-card', 'mapp-alert-card', 'mapp-guided-test-panel', 'mapp-assisted-execution-panel', 'mapp-triage-panel']) {
...
console.log('OK: release_check v129 PWA passou. Execução real assistida, teste guiado, permissões por papel e Supabase preservado.');
