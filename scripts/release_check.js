...
const currentVersion = 'pwa-supabase-v139-painel-executivo-saude';
const currentCache = 'smart-loja-pwa-supabase-v139-painel-executivo-saude';
...
if (!mainSource.includes('smart-mobile-rebuild-v139')) fail('main.tsx precisa aplicar a classe smart-mobile-rebuild-v139.');
...
if (!webApiSource.includes(currentCache)) fail('WEB_CACHE_VERSION precisa estar no cache v139 feedback do cliente e NPS.');
if (!serviceWorkerSource.includes(currentCache)) fail('Service worker precisa usar cache v139 feedback do cliente e NPS.');
...
for (const token of ['mapp-root', 'mapp-bottom-nav', 'mapp-sidebar', 'mapp-page', 'mapp-stat-card', 'mapp-alert-card', 'mapp-guided-test-panel', 'mapp-assisted-execution-panel', 'mapp-triage-panel', 'mapp-final-release-panel', 'mapp-demo-panel', 'mapp-tour-panel', 'mapp-proposal-panel', 'mapp-client-feedback-panel']) {
...
console.log('OK: release_check v139 PWA passou. Painel executivo de saúde comercial, feedback/NPS e melhorias prioritárias, pós-venda, termo de implantação, proposta comercial, tour guiado, modo treinamento, onboarding, aceite final, permissões por papel e Supabase preservado.');
